...
FROM base AS build
WORKDIR /app
# rustup doesn't install a C toolchain the way Debian's packaged rustc did
# (that package hard-depends on gcc + libc6-dev for linking). Restore just
# those two, matching --no-install-recommends below so gcc's own Recommends
# don't reintroduce the packaged rust we're replacing.
RUN apt-get update \
...
# Keep this under server/node_modules so both CommonJS resolution and the
# native ESM dynamic import used by the Sentry gate find the package.
COPY --chown=node:node --from=server-deps /app/.server-deps/node_modules /app/server/node_modules
